...
_llm_local_path = Path(__file__).resolve().parent / "lllm-local.py"
_spec = importlib.util.spec_from_file_location("llm_local", _llm_local_path)
_llm_local = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_llm_local)
FrameDescriber = _llm_local.FrameDescriber
OLLAMA_URL = _llm_local.OLLAMA_URL
OLLAMA_MODEL = _llm_local.OLLAMA_MODEL
...
def describe_frames_after_predict_each_frame(
    ...
    """Describe frames using Ollama or BLIP. Delegates to llm-local.FrameDescriber.
